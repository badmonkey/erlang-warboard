...
-export([start_link/1, child_spec/2, server_name/1]).
...
-record(state,
    { evtchannel
    , presence_table        :: ets:tid()
    , login_table           :: ets:tid()
    , world                 :: war_type:world()
    , first_event           :: war_type:timestamp()
    , last_update           :: war_type:timestamp()
    , oldest_player         :: { war_type:player_id(), war_type:timestamp() } | undefined
...
start_link(World) ->
    startphase:start_link_name(server_name(World), ?MODULE, [World]).

    
child_spec(Id, Args) -> ?SERVICE_SPEC(Id, ?MODULE, Args).


server_name(briggs) -> war_presence_briggs;
server_name(jaeger) -> war_presence_jaeger.
...
-spec population( war_type:world() ) -> {non_neg_integer(), non_neg_integer(), non_neg_integer()}.

population(World) ->
    gen_server:call( server_name(World), {population} ).
    

-spec oldest_player( war_type:world() ) -> {war_type:player_id(), war_type:timestamp()} | undefined.
